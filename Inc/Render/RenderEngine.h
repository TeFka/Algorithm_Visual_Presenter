...
private:
    ...
    ShaderPipeline<glm::vec3> skyboxShadProgram;
    ...
    const int defaultShaderNum = 8;

    std::string defaultShaderNames[12] = {"test_vert.glsl", "test_frag.glsl",
...
public:
    ...
    const int SKYBOX_SHADER = 3;
    const int BASIC_2D_SHADER = 4;
    const int BASIC_3D_SHADER = 5;
    const int BASIC_TRANSPARENT_2D_SHADER = 6;
    const int BASIC_TRANSPARENT_3D_SHADER = 7;
                         ...
                         int useGeometryShader = 0, const char* geometryFile = "")
    {
        ...
        return (this->shaderPrograms.size()-this->defaultShaderNum-1);
    }

    void setDefaultShaderUniforms();

    void setupSkyBoxTexture(std::vector<std::string>);

    void setSkyBoxTexture();
    void unSetSkyBoxTexture();
