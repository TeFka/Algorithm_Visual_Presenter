...
void InterfaceLayout::setMembersParameters()
{
    ...
    if(this->orient == HORIZONTAL)
    {
        for(int i = 0; i<this->widgets.size(); i++)
        {
            if(this->widgets[i]->isEnabled())
            {
                this->widgets[i]->setHeightHint(1.0);
        ...
        for(int i = 0; i<this->widgets.size(); i++)
        {
            if(this->widgets[i]->isEnabled())
            {
                if(this->widgets[i]->isRelative())
                {
                    this->widgets[i]->setWidthHint((freeSpace/relativeNum));
    ...
    else if(this->orient == VERTICAL)
    {
        ...
        for(int i = (this->widgets.size()-1); i>=0; i--)
        {
            if(this->widgets[i]->isEnabled())
            {
                this->widgets[i]->setWidthHint(1.0);
        ...
        for(int i = (this->widgets.size()-1); i>=0; i--)
        {
            if(this->widgets[i]->isEnabled())
            {
                if(this->widgets[i]->isRelative())
                {
                    this->widgets[i]->setHeightHint((freeSpace/relativeNum));
