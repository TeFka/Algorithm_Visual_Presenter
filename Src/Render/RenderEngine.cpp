...
void  RenderEngine::updateViewport()
{
    ...
    if(this->windowWidth != this->framebufferWidth||this->windowHeight != this->framebufferHeight)
    {
        ...
        std::cout<<"new size: "<<this->windowWidth<<" "<<this->windowHeight<<std::endl;
...
void RenderEngine::setupSkyBoxTexture(std::vector<std::string> faces)
{
...
void RenderEngine::setSkyBoxTexture(){

    this->backgroundExists = true;

}

void RenderEngine::unSetSkyBoxTexture()
{
                               ...
                               std::string shareFolder)
{
    ...
    std::vector<verticeInfo*> skyboxVertsInf = {new verticeInfo{2,3}};
    this->skyboxShadProgram.setSource((shareFolder+this->defaultShaderNames[6]).c_str(),(shareFolder+this->defaultShaderNames[7]).c_str());
    this->skyboxShadProgram.setVerticeInfo(skyboxVertsInf);
    this->skyboxShadProgram.setupVertex();
    this->shaderPrograms.push_back(&skyboxShadProgram);

    this->shaderPrograms[3]->addVertex(glm::vec3(-1.0f,  1.0f, -1.0f));
    this->shaderPrograms[3]->addVertex(glm::vec3(-1.0f, -1.0f, -1.0f));
    this->shaderPrograms[3]->addVertex(glm::vec3(1.0f, -1.0f, -1.0f));
    this->shaderPrograms[3]->addVertex(glm::vec3(1.0f, -1.0f, -1.0f));
    this->shaderPrograms[3]->addVertex(glm::vec3(1.0f,  1.0f, -1.0f));
    ...
    this->basic2DShadProgram.setSource((shareFolder+this->defaultShaderNames[8]).c_str(),(shareFolder+this->defaultShaderNames[9]).c_str());
    ...
    this->basic3DShadProgram.setSource((shareFolder+this->defaultShaderNames[10]).c_str(),(shareFolder+this->defaultShaderNames[11]).c_str());
    ...
    this->basicTransparent2DShadProgram.setSource((shareFolder+this->defaultShaderNames[8]).c_str(),(shareFolder+this->defaultShaderNames[9]).c_str());
    ...
    this->basicTransparent3DShadProgram.setSource((shareFolder+this->defaultShaderNames[10]).c_str(),(shareFolder+this->defaultShaderNames[11]).c_str());
...
void RenderEngine::renderSkyboxBackground(glm::mat4 theViewMat)
{
    if(this->backgroundExists == true)
    {
        glDepthFunc(GL_LEQUAL);
        glDepthMask(GL_FALSE);
        this->shaderPrograms[SKYBOX_SHADER]->getShader()->set1i(0,"cubText");
        this->shaderPrograms[SKYBOX_SHADER]->getShader()->setMat4(theViewMat,"viewMatrix");
        glActiveTexture(GL_TEXTURE0);
        glBindTexture(GL_TEXTURE_CUBE_MAP, this->skyBoxTexture);
        this->shaderPrograms[SKYBOX_SHADER]->use();
        glDepthFunc(GL_LESS);
        glDepthMask(GL_TRUE);
    }
}
