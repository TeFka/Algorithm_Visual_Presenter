...
{
    this->updateVertex(verticesIndex);
    glBindVertexArray(this->VAO);
    if(useSubVBO)
    {
        glBindBuffer(GL_ARRAY_BUFFER,this->VBO);
        glBufferSubData(GL_ARRAY_BUFFER,0,sizeof(verticeType)*this->vertices[verticesIndex].size(),this->vertices[verticesIndex].data());
    }
    this->theShader->use();
    ...
    if(this->type == TRIANGLES)
    {
        ...
        glDrawArrays(GL_TRIANGLES,0,this->vertices[verticesIndex].size());

    }
    else if(this->type == POINTS)
    {

        glDrawArrays(GL_POINTS, 0, this->vertices[verticesIndex].size());
...
void ShaderPipeline<verticeType>::clearVerts(int verticesIndex)
{
    ...
    this->vertices[verticesIndex].clear();
