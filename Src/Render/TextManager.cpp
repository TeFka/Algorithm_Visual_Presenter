...
void TextManager::writeText(std::string text, GLfloat wPart, GLfloat hPart, GLfloat maxHeight, GLfloat scale,glm::vec3 color)
{
    ...
    for(c=text.begin(); c!=text.end(); c++)
    {
        this->renderEngine->getShaderProgram(2, true)->clearVerts();
        Character ch = this->characters[*c];
        ...
        this->renderEngine->getShaderProgram(2, true)->addVertex(glm::vec4(xPos,yPos,0.0,1.0));
        this->renderEngine->getShaderProgram(2, true)->addVertex(glm::vec4(xPos+chW,yPos,1.0,1.0));
        this->renderEngine->getShaderProgram(2, true)->addVertex(glm::vec4(xPos,yPos+chH,0.0,0.0));

        this->renderEngine->getShaderProgram(2, true)->addVertex(glm::vec4(xPos,yPos+chH,0.0,0.0));
        this->renderEngine->getShaderProgram(2, true)->addVertex(glm::vec4(xPos+chW,yPos,1.0,1.0));
        this->renderEngine->getShaderProgram(2, true)->addVertex(glm::vec4(xPos+chW,yPos+chH,1.0,0.0));

        glBindTexture(GL_TEXTURE_2D,ch.text);

        this->renderEngine->getShaderProgram(2, true)->use();
                               ...
                               GLfloat wLimit, GLfloat hLimit, glm::vec3 color, bool relative)
{
    ...
    glDepthFunc(GL_LESS);
    glDepthMask(GL_TRUE);
    this->renderEngine->getShaderProgram(2, true)->getShader()->setVec3f(color,"textCol");
